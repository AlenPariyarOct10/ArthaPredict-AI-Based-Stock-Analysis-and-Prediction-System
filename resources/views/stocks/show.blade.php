...
@section('content')
    @if(session('success'))
        <div
            class="mb-6 rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 px-4 py-3 text-green-700 dark:text-green-300">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
            <h3 class="text-gray-700 dark:text-gray-200 text-3xl font-bold flex items-center">
                ...
                <span
                    class="ml-4 text-sm px-3 py-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg">{{ $stock->exchange }}</span>
            </h3>
        ...
        <div class="mt-4 md:mt-0 flex space-x-2">
            <form action="{{ route('stocks.run_ma', $stock->symbol) }}" method="POST">
                @csrf
                <button type="submit"
                    class="px-4 py-2 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-100 shadow-sm rounded-lg hover:bg-blue-100 dark:hover:bg-blue-800 transition flex items-center">
            ...
            <form action="{{ route('watchlist.toggle') }}" method="POST">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 shadow-sm rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition flex items-center">
    ...
    <div class="mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @forelse($predictions as $prediction)
                <div
                    class="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm relative overflow-hidden group">
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 opacity-0 group-hover:opacity-100 transition duration-300 z-0">
                    </div>
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 text-sm">
                                ...
                                @else
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ (string) $prediction->additional_metrics }}</span>
                                    </div>
                                @endif
                            </div>
                        @endif
            ...
            @empty
                <div
                    class="md:col-span-3 rounded-xl border border-dashed border-gray-300 dark:border-gray-600 p-8 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800">
